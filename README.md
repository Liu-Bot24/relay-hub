...
- 开启可接单状态：
  `python3 scripts/agent_relay.py --agent <你的 agent_id> set-presence --status ready`
- 关闭可接单状态：
  `python3 scripts/agent_relay.py --agent <你的 agent_id> set-presence --status offline`
...
- `<your-agent-id>` 就是你自己的对象名，例如 `claude-code`、`codex`、`gemini-cli`、`cursor-cli`、`opencode`
