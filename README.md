...
2. web_base_url
- 自动探测本机局域网内可访问的 IPv4 地址，并组装成 http://<LAN_IP>:4317
- 优先使用 192.168.x.x、10.x.x.x、172.16-31.x.x 这类局域网地址
- 只有在确实无法可靠判断时，才来问我
...
- 如果你是 Claude Code，执行：
  cd <仓库路径>
  python3 install.py full --web-base-url <你探测到的 LAN URL> --delivery-channel feishu=ou_xxx --delivery-channel openclaw-weixin=xxx@im.wechat --delivery-account openclaw-weixin=your-account-id --worker-agent claude-code --worker-backend claude-code --load-services

- 如果你不是 Claude Code，执行：
  cd <仓库路径>
  python3 install.py full --web-base-url <你探测到的 LAN URL> --delivery-channel feishu=ou_xxx --delivery-channel openclaw-weixin=xxx@im.wechat --delivery-account openclaw-weixin=your-account-id --worker-agent <你的 agent_id> --worker-backend manual --load-services

说明：
- 上面的 feishu、openclaw-weixin 只是示例渠道；如果用户给的是别的 OpenClaw 渠道，就替换成真实 channel=target
- 如果某个渠道需要账号参数，再加 --delivery-account channel=accountId
- 如果用户只给了一个渠道，就只保留那一个 --delivery-channel
- manual 的意思是：仓库不为你内置现成自动 worker，但 OpenClaw 和网页入口会先安装好；后续你再按 docs/AGENT_ENTRY_RULE.md 和 docs/AGENT_WORKFLOW.md 接入

安装完成后，再执行：
1. python3 install.py doctor --web-base-url <你探测到的 LAN URL> --delivery-channel feishu=ou_xxx --worker-backend <claude-code 或 manual>
