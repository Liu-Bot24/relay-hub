...
正常安装目标就是：在同一份本地仓库副本里一次完成安装。为了“绕过问题”再克隆第二份、副本漂移到另一份同名仓库、或改到临时目录继续安装，都属于安装器偏离流程，不是正常主路径。
...
- 只有明确启用了某个宿主的可选增强组件，并且该组件确实提供了当前会话解析能力时，才可能在特定路径下自动省略其中一部分参数；通用主路径不要假设这一点
...
- 通用主路径默认只要求：用户回到主窗口后，可以通过产品命令 `合流上下文` 把 branch 增量接回主线
- 不要把“自动合流”当成所有宿主的默认硬要求
- 只有当前宿主确实支持可靠的“主窗口用户发言前”触发机制，并且你已经把这条链路真实写进宿主持久载体后，才允许把“回主窗口自动先合流”报告为已完成
- 如果当前宿主没有这类可靠前置机制，安装完成后必须明确提醒用户：回到主窗口时先说 `合流上下文`，再继续主线
