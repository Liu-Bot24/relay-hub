...
- 当 OpenClaw 或外部 AI 发同步提醒时，提醒消息会自动附带网页入口和固定产品操作提示；如果当前主会话已经绑定了 OpenClaw 来源对象，就优先复用该来源对象；如果这是首次从宿主主窗口开启且还没有绑定来源对象，就默认发送到当前已启用的 OpenClaw 消息渠道；`打开 <agent> 入口` 仍保留为显式重发入口或会话管理动作
- `main_context.md` 的生成与 `merge-back` 的消费，原则上不由 OpenClaw 负责
- OpenClaw 只管理 branch 的打开、触发和发送
