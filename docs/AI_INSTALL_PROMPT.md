...
先判断当前机器平台，并直接执行对应命令，不要把占位符原样发出来：
- macOS / Linux：执行 `python3 install.py install-host --load-services`
- Windows：执行 `py -3 install.py install-host --load-services`
- 如果 Windows 环境里已经有稳定可用的 `python3`，也可以执行 `python3 install.py install-host --load-services`
...
安装完成后，再执行：
1. macOS / Linux：`python3 install.py doctor`
   Windows：`py -3 install.py doctor`
2. macOS / Linux：`python3 install.py status`
   Windows：`py -3 install.py status`
...
- 如果我后面明确要求覆盖默认发现结果，或额外指定某个固定目标，那时你再执行：
  macOS / Linux：`python3 install.py install-openclaw --delivery-channel channel=target --delivery-account channel=accountId`
  Windows：`py -3 install.py install-openclaw --delivery-channel channel=target --delivery-account channel=accountId`
