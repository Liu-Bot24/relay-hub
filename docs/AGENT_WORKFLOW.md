...
如果当前主会话还没有任何可复用的 OpenClaw 渠道对象，脚本会自动回退到当前 OpenClaw 已启用的默认提醒渠道；只有当这两者都不存在时，这一步才应明确告诉用户“提醒已跳过”，而不是假装发送成功。只有你明确知道当前不该发启动提醒时，才额外加上：
...
- 对通用宿主，这一步必须被接进宿主环境里的持久自动机制，保证每条主窗口正常回复后都会自动触发；这条自动机制既可以是原生 hook，也可以是宿主持久规则里的“回复收尾流程”。单次手动补跑只算补救。
