...
- 如果当前没有配置额外镜像渠道，OpenClaw 侧安装或状态汇报只应如实说明“默认仍走原始触发渠道”
...
6. `enable-relay` 默认就会自动发送一条不创建 branch 的启动提醒；提醒里会自动附带网页入口和固定产品操作提示，并优先复用当前主会话已经绑定的 OpenClaw 渠道对象；如果没有可复用来源，再回退到已配置的额外提醒渠道
