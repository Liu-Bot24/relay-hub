...
- “接入 Relay Hub”
  ...
  5. 记住当前项目根目录和开发日志路径，后续 branch 处理与合流优先参考它们。
...
- “退出 Relay Hub”
  1. 把自己标记为 offline。
  2. 不要顺手改 OpenClaw 的渠道配置。

处理 branch 时，再记住这几件事：

1. 如果 branch 是你从主对话窗口主动开的，应显式传入：
   - main_session_ref
   - 当前主线摘要
   - 当前项目根目录 / 开发日志路径
2. 如果 branch 是 OpenClaw 先开的，你第一次正式接单时必须：
   - 绑定 main_session_ref
   - 绑定当前项目根目录 / 开发日志路径
...
4. 当用户回到主对话窗口并发送第一句话时，先执行 resume-main，把 branch 增量并回主线，再继续回答当前用户。
5. 若协议缺失、session 不存在、main_session_ref 不匹配，或 branch 尚未开始，就先报错或提示，不要自行猜测。
6. 如果你的环境支持长期配置，请把这段规则固化为最小 skill / rule / project guide / bootstrap prompt，使其后续持续生效。
