...
- “接入 Relay Hub”
  ...
  4. 立即写入一条主线快照，作为 Relay Hub 启用时的上下文基线。
     ...
     - 如果宿主拿不到像 Codex rollout 这样完整的原文：
       - 退回到宿主可见上下文的最佳努力快照；
  ...
  10. 只有在持续接单机制已经运行后，才能把自己视为完整 ready；如果你的环境根本做不到持续接单，就明确告诉用户当前只能 manual-only。
...
7. 只要当前仍处于 Relay Hub 已接入状态，主窗口里的正常回复默认也应镜像成提醒；如果宿主支持代码级原文捕获，就必须优先做精确镜像，不要再生成第二份措辞相近的新文本。Codex 走 rollout 原文捕获；其他宿主默认走“精确正文捕获队列”：在最终正文已经产出后，把同一份正文写入临时文件，再执行 `capture-main-output --body-file <exact_body_file>`。
...
- `scripts/agent_relay.py capture-main-output` 是通用保底入口：把已经确定的最终正文排进 pickup 守护的精确镜像队列，由代码转发到 OpenClaw
